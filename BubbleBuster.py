...
def update_mouse_angle():
    ...
    mouse_angle = math.degrees(
        math.atan2(
            (player.y + level.bubble_height / 2) - position_y,
...
def place_bubble():
    ...
    level.matched_bubbles.clear()
    level.match_3(grid_pos[0], grid_pos[1])
    if len(level.matched_bubbles) >= 3:
        for match in level.matched_bubbles:
            match.bubble_color = -1
            match.to_remove = False
    else:
        for match in level.matched_bubbles:
            match.to_remove = False
    if level.check_lose() or level.check_win():
        return
    level.check_colors_left()
    bubble_to_shoot.reinitialize(
        SCREEN_WIDTH / 2 - BUBBLE_WIDTH / 2,
        SCREEN_HEIGHT - 2.5 * BUBBLE_WIDTH,
        500,
        90,
        random.choice(tuple(level.colors_in_level)),
        False,
    )
    return
...
def mooving_bubble(delta_time):
    bubble_to_shoot.x += (
        delta_time
        * bubble_to_shoot.speed
        * math.cos(math.radians(bubble_to_shoot.angle))
    )
    bubble_to_shoot.y += (
        delta_time
        * bubble_to_shoot.speed
        * -1
        * math.sin(math.radians(bubble_to_shoot.angle))
    )
    ...
    for i in range(level.rows):
        for j in range(level.columns):
            ...
            if collision_detection(
                bubble_to_shoot.x + level.bubble_width / 2,
...
def draw_limits():
    pygame.draw.rect(
        window,
        (0, 0, 0),
        (level.x, 0, level.x + (level.columns - 1) * level.bubble_width, level.y),
    )
    pygame.draw.line(
        window,
        (0, 0, 0),
        (level.x, level.y),
        (level.x, level.y + (level.rows - 1) * level.row_height),
        2,
    )
    pygame.draw.line(
        window,
        (0, 0, 0),
        (
            level.x + level.columns * level.bubble_width + 6,
            level.y,
        ),
        (
            level.x + level.columns * level.bubble_width + 6,
            level.y + (level.rows - 1) * level.row_height,
        ),
        2,
    )
    pygame.draw.line(
...
def main():
    ...
    while run:
        ...
        level.remove_floating_bubbles()
        if counter == 5:
            level.y += level.bubble_width
            level.rows -= 1
            counter = 0
